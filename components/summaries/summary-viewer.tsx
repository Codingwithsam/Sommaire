...
const SectionTile = ({title}: { title: string }) => {
    return (
...
const SummaryViewer = ({summary}: { summary: any }) => {
    const [currentSection, setCurrentSection] = useState(0)
    const handleNext = () => setCurrentSection((prev) => Math.min(prev + 1, sections.length - 1))
    const handlePrevious = () => setCurrentSection((prev) => Math.min(prev - 1, sections.length - 1))

    const sections = summary.split('\n# ').map((section: any) => section.trim()).filter(Boolean).map(parseSection)
        ...
        <Card
            ...
            <MotionDiv
                ...
                <div className={"px-4 sm:px-6"}>
                    <SectionTile title={sections[currentSection]?.title || ""}/>
            ...
            <NavigationControls currentSection={currentSection} totalSections={sections.length}
                                onPrevious={handlePrevious}
                                onNext={handleNext} onSectionSelect={setCurrentSection}/>
        </Card>
    )
}
