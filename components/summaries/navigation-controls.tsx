...
const NavigationControls = ({currentSection, totalSections, onPrevious, onNext, onSectionSelect}: {
    ...
    return (
        <div
            className={"absolute bottom-0 left-0 right-0 p-4 bg-background/80 backdrop-blur-xs border-t border-rose-500/10"}>
                ...
                <div className={"flex gap-2"}>
                    {Array.from({length: totalSections}).map((_, index) => (
                        <button
                            ...
                            className={cn("h-2 w-2 rounded-full transition-all duration-300", currentSection === index ? "bg-linear-to-r from-rose-500 to-rose-600" : "bg-rose-500/20 hover:bg-rose-500/30")}
                        />
                    ))}
                </div>
