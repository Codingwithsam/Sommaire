            ...
            <div className={"px-4 flex gap-1.5"}>
                {
                    sections.map((_, index) => (
                        <div className={"h-1.5 flex-1 rounded-full bg-rose-500/10 overflow-hidden"}
                             key={index}>
                            <div
                                className={cn("h-1.5 bg-linear-to-r from-gray-500 to-rose-600 transition-all duration-500", index === currentSection ? "w-full" : currentSection > index ? "w-full opacity-10" : "w-0")}
                            />
                        </div>
                    ))
                }
