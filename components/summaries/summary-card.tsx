...
const StatusBadge = ({status}: { status: string }) => {
    return <span
        className={cn("px-3 py-1 text-xs font-medium rounded-full capitalize", status === "completed" ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800")}>{status}</span>
}
    ...
    return (
        <div>
            <Card className={"relative h-full"}>
                ...
                <Link href={`summaries/${summary.id}`} className={"block px-4 sm:px-6 -mt-2"}>
                    <div className={"flex flex-col gap-3 sm:gap-4"}>
                        <SummaryHeader fileUrl={summary.original_file_url} title={summary.title}
